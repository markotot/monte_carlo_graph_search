...
from monte_carlo_graph_search.environment.minigrid.custom_minigrid_env import CustomMinigridEnv
...
def run_app(config: DictConfig) -> None:
    ...
    total_reward = 0
    for i in range(config.search.max_moves):
    ...
    plot_images(f"env seed: {config.env.seed}   agent seed: {config.search.seed}", images, total_reward)
